...
def get_ams(p_PDFInterpolMVAPdf, p_NsmoothMVAPdf):
  #some checks to make debugging easier, just in case
  if type(p_PDFInterpolMVAPdf)!=int:
    raise ValueError("p_PDFInterpolMVAPdf must be positive integer")
  if type(p_NsmoothMVAPdf)!=int:
    raise ValueError("p_NsmoothMVAPdf must be positive integer")
  ...
  file_name_appendix="_spline"+str(p_PDFInterpolMVAPdf)+"_smooth"+str(p_NsmoothMVAPdf)
...
if __name__ == '__main__':
  ...
  with open(out_file_name, "a") as out_file:
    for PDFInterpolMVAPdf in PDFInterpolMVAPdfs:
      for NsmoothMVAPdf in NsmoothMVAPdfs:
	ams=get_ams(PDFInterpolMVAPdf, NsmoothMVAPdf)
	out_file.write(str(PDFInterpolMVAPdf)+"\t"+str(NsmoothMVAPdf)+"\t"+str(ams[0])+"\t"+str(ams[1])+"\n")
	out_file.flush()
