...
# this defines the algorithms and trainig options     
    ...
    global_options = 'H:V:CreateMVAPdfs:NbinsMVAPdf=100:'+additional_options
...
#                      useful if plots are to ge generated without displaying them
    ...
    for var_transform_option in var_transform_options:
        file_name_appendix=method_name+"_"+var_transform_option
        ...
        train(infile, 
              TMVAoutfile, 
              additional_options="VarTransform="+var_transform_option+":")
        print "evaluation---------------------------------------------------------"
        evaluate(infile,resultfile)
        print "analyse---------------------------------------------------------"
        maxams = analyse(resultfile)
        print""
        results.append([var_transform_option, maxams[0], maxams[1]])
    
    results=sorted(results, key=lambda x:x[1])    
    with open(out_file_name, "w") as out_file:   
        for result in results:
            out_file.write(result[0]+"\t"+str(result[1])+"\t"+str(result[2])+"\n")
