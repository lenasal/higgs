...
#shrinks = np.arange(0.05,0.1,0.001)  
Depths = [7,8,9,10]
NTs = [200,300,400,500]
for NT in NTs:
  ams_array = []
  for D in Depths:
	...
	if (root_version == '5.34' and patch_number < '11') or root_version < '5.34':
	    tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":nEventsMin=2%:BoostType=Grad:Shrinkage=0.07:UseBaggedGrad:GradBaggingFraction=0.5:nCuts=200:MaxDepth="+str(D))
	else:
	  tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":MinNodeSize=2%:BoostType=Grad:Shrinkage=0.07:UseBaggedBoost:GradBaggingFraction=0.5:nCuts=200:MaxDepth="+str(D))
  ...
  plt.plot(Depths,ams_array,label="NTrees "+str(NT))
