...
for NT in NTs:
  ...
  for s in shrinks:
	...
	if (root_version == '5.34' and patch_number < '11') or root_version < '5.34':
	    tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":nEventsMin=2%:BoostType=Grad:Shrinkage="+str(s)+":UseBaggedGrad:GradBaggingFraction=0.5:nCuts=500:MaxDepth=5")
	else:
	  tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":MinNodeSize=2%:BoostType=Grad:Shrinkage="+str(s)+":UseBaggedBoost:GradBaggingFraction=0.5:nCuts=500:MaxDepth=5")
    ...
    evaluate()
    analyse()
  np.savetxt(str(NT)+".txt", np.transpose([shrinks,ams_array]))
  plt.plot(shrinks,ams_array,label="NTrees "+str(NT))
