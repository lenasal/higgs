  ...
  for shrink in shrinks:
	...
	for depth in depths:
	  ...
	  # this defines the algorithms and trainig options     
	      ...
	      plot_filename += "NTrees_"
	      plot_filename += str(shrink) +str(Cut) + str(Event)
		...
		tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":MinNodeSize="+str(Event)+"%:BoostType=Grad:Shrinkage="+str(shrink)+":UseBaggedBoost:GradBaggingFraction=0.5:nCuts="+str(Cut)+":MaxDepth="+str(depth))
	      else:
		tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":MinNodeSize="+str(Event)+"%:BoostType=Grad:Shrinkage="+str(shrink)+":UseBaggedBoost:GradBaggingFraction=0.5:nCuts="+str(Cut)+":MaxDepth="+str(depth))
	  ...
	  evaluate()
	  analyse()
	np.savetxt(str(NT)+".txt", np.transpose([depths,ams_array]))
	plt.plot(NTs,ams_array)

#obal_options + "NTrees="+str(NT)+":nEventsMin=2%:BoostType=Grad:Shrinkage=0.08:UseBaggedGrad:GradBaggingFraction=0.5:nCuts=200:MaxDepth="+str(depth)
plt.xlabel("Depth")
plt.ylabel("AMS") 
plt.legend(loc=4)
