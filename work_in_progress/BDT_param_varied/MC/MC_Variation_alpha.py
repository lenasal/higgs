...
def train(shrinkage,Depth,NT,nEventsMin,nCuts,feature_filename = 'atlas-higgs-challenge-2014-v2_part.root', 
...
# Open ROOT files and create TMVA factory
  ...
  if (root_version == '5.34' and patch_number < '11') or root_version < '5.34':
      tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":nEventsMin="+str(nEventsMin)+":BoostType=Grad:Shrinkage="+str(shrinkage)+":UseBaggedGrad:GradBaggingFraction=0.5:nCuts="+str(nCuts)+":MaxDepth="+str(Depth))
  else:
    tmva_factory.BookMethod(ROOT.TMVA.Types.kBDT, "BDT", global_options + "NTrees="+str(NT)+":MinNodeSize=2%:BoostType=Grad:Shrinkage="+str(shrinkage)+":UseBaggedBoost:GradBaggingFraction=0.5:nCuts="+str(nCuts)+":MaxDepth="+str(Depth))
