...
#            "PRI_jet_subleading_pt","PRI_jet_subleading_eta","PRI_jet_subleading_phi","PRI_jet_all_pt"]
  ...
  return current_selection_of_variables
...
# this defines the algorithms and trainig options     
    ...
    tmva_factory.BookMethod(ROOT.TMVA.Types.kLikelihood, "Likelihood", global_options +\
...
def analyse(result_filename = 'result.csv'):
    ...
    return maxams_L,maxams_F,maxams_BDT,maxams_MLP
...
#                      useful if plots are to ge generated without displaying them
    ...
    ams_outfile_name="the_results.dat"

    global all_variables
    global current_selection_of_variables
    current_selection_of_variables=[]
    
    with open(ams_outfile_name, "w") as ams_results_file:
      for variable_ommited in all_variables:
	current_selection_of_variables=copy.deepcopy(all_variables)
	current_selection_of_variables.remove(variable_ommited)
	TMVAoutfile='TMVAout_'+variable_ommited+'.root'
	resultfile='result_'+variable_ommited+'.csv'
	...
	print "analyse---------------------------------------------------------"
	maxams_L,maxams_F,maxams_BDT,maxams_MLP = analyse(resultfile)
	ams_results_file.write(str(maxams_L) + "\t" + str(maxams_F) + "\t" + str(maxams_BDT)+ "\t" + str(maxams_MLP) + "\t" + variable_ommited +  "\n")
